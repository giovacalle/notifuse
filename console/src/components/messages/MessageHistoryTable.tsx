      ...
      render: (record: MessageHistory) => {
        return (
          <>
            ...
            <span className="text-xs text-muted-foreground pl-2">v{record.template_version}</span>
          </>
        )
      }
    },
        ...
        if (!broadcast) {
          return (
            <Tooltip title={broadcastId}>
              <span className="text-xs text-muted-foreground">{broadcastId.substring(0, 8)}...</span>
            </Tooltip>
          )
        }
  ...
  return (
    <>
      <Table
        ...
        pagination={false}
        size={size}
        className={bordered ? 'border border-border-base rounded' : ''}
      />
